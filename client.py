        ...
        with open("/proc/diskstats", "r", encoding="utf-8") as f:
            for line in f:
                ...
                if dev.startswith(("loop", "ram")):
                    continue
                if any(ch.isdigit() for ch in dev[1:]):
                    if dev.startswith("nvme") and "p" not in dev:
                        pass
                    else:
                        if dev.startswith(("xvd", "sd", "vd")):
                            if dev[-1].isdigit():
                                continue
                        else:
                            continue
...
def monitor_loop(url: str, poll_seconds: int, stop_event: threading.Event) -> None:
    ...
    while not stop_event.is_set():
        try:
            ...
            if data is None:
                print(f"⚠️ 서버 에러: HTTP {latency:.3f}s (status != 200)")
            else:
                ...
                if isinstance(curr_read, (int, float)) and isinstance(curr_write, (int, float)):
                    if prev_read is None or prev_write is None or prev_ts is None:
                        read_speed = 0.0
                        write_speed = 0.0
                    else:
                        dt = now - prev_ts
                        read_speed = mb_per_sec(int(curr_read) - int(prev_read), dt)
                        write_speed = mb_per_sec(int(curr_write) - int(prev_write), dt)
            ...
            print("🚨 연결 실패! (서버/보안그룹/방화벽 확인)")
        except requests.exceptions.Timeout:
            print("🚨 타임아웃! (네트워크 지연/서버 응답 지연)")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        choices=["agent", "poll"],
        default="agent",
        help="agent: 이 머신에서 /monitor 제공(사이트가 연결) | poll: 원격 /monitor 폴링 + 로컬 aws cli",
    )
